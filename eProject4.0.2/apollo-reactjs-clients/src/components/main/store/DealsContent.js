...
function DealsContent() {
    ...
    async function fetchData() {
        ...
        await axios
            .get(`http://localhost:9999/api/stores/${storeInfo.id}/products`)
            .then((res) => {
    ...
    return (
        <div className="font-shopFont">
            <div className="w-full h-full text-center pt-4 text-3xl font-light text-gray-600"> Featured Deals </div>
              ...
              {typeof product.category === 'object' ? JSON.stringify(product.category) : product.category}
                        ...
                        <div className="w-full h-auto flex items-center justify-center relative group">
                            ...
                            <ul className="w-full h-36 bg-gray-100 absolute bottom-[-170px] flex flex-col items-end justify-center gap-2 font-titleFont px-2 border-1 border-r group-hover:bottom-0 duration-700">
                                ...
                                <Link to={`/product/${product.id}`} className="productLi"> View Details <span><ArrowCircleRightIcon/></span> </Link>
