...
function ProductDetail() {
    ...
    useEffect(() => {
        if (variantRender0) {
            // Thêm kiểm tra nếu variantRender0.imageDTOList và phần tử đầu tiên của nó tồn tại
        ...
        return (<div className="font-bodyFont w-full bg-gray-100 p-1">
            <div className="container mx-auto h-auto grid grid-cols-5 gap-2">
                ...
                <div
                    ...
                    <div
                        className="font-titleFont px-8 mx-8 my-4 hover:border-spacing-x-5 text-center justify-center tracking-wide text-green-900 size text-sm ">
                    ...
                    <div>
                        <div className="flex flex-wrap text-center justify-between object-contain hover:py-4 mx-21">
                            ...
                            {variantRender && variantRender.imageDTOList?.map((item, index) => (<img
                                key={index}
                            ...
                            ></img>))}
                            {variantRender && variantRender.videoDtoList?.map((item, i) => (<video
                                key={i}
                                controls
                        ...
                        {variantDetail.optionTableDto.map((option, i) => (<div key={i}>
                            ...
                            <div
                                ...
                                <div className="space-x-4 flex text-xl">
                                    {option.optionValueDTOList.map((ele, index) => (variantRender.optionValueDTOList[i].id == ele.id ? (
                                        <button key={index} onClick={() => changeValueId(i, ele.id)}
                                                className=" text-sm w-16 h-10 rounded-sm text-center text-slate-70 bg-slate-950 text-white border-neutral-200 border-5"> {ele.value}</button>) : (
                                        <button key={index} onClick={() => changeValueId(i, ele.id)}
                                                className=" text-sm w-16 h-10 rounded-sm text-center text-slate-70 bg-gray-300 text-white"> {ele.value}</button>)))}
                                </div>
                            </div>
                        </div>))}
